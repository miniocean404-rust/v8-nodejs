...
impl AsyncTaskDispatcher for TokioAsyncTaskManager {
        ...
        F: Future<Output = Self::AsyncTaskResult> + Send + 'static,
    {
        ...
        tokio::spawn({
            ...
            async move {  // 异步闭包
                let task_value = async_block.await;  // 等待异步块完成
                let task_message = AsyncTaskMessage {
                    task_id,
                    payload: task_value,  // 结果
                };
    ...
    async fn run_event_loop(&mut self, isolate: &mut v8::Isolate, scope: &mut v8::HandleScope<'_>) {
        while let Some(message) = self.channel_receiver.recv().await {  // 接收任务完成消息
            if let Some((_, task)) = self.tasks.remove(&message.task_id) {  // 从存储中移除任务
                let promise_resolver = unsafe { Global::from_raw(isolate, task.promise_resolver) };  // 还原 Promise 解析器

                // 根据结果类型处理
                match message.payload {
                    AsyncTaskResult::Resolve(task_value) => {  // 成功
                        let v8_value = task_value.into_v8(scope);  // 转换为 V8 值
                        promise_resolver.open(scope).resolve(scope, v8_value);  // Resolve Promise
                    }
                    AsyncTaskResult::Reject(task_value) => {  // 失败
                        let v8_value = task_value.into_v8(scope);
                        promise_resolver.open(scope).reject(scope, v8_value);  // Reject Promise
                    }
                }

                isolate.perform_microtask_checkpoint();  // 执行微任务检查点
            }
        }
    }
}
...
impl AsyncTaskValue {
    ...
    pub fn into_v8<'s>(self, scope: &mut v8::HandleScope<'s>) -> v8::Local<'s, v8::Value> {
        match self {
            AsyncTaskValue::String(value) => {  // 如果是字符串
